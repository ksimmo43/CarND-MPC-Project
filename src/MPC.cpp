...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The part of the cost based on the reference state. Larger weight was
    // on cte and epsi.
    ...
    // Minimize the use of actuators. Heigher weight placed on steering to smooth control
    for (unsigned int t = 0; t < N - 1; t++) {
      fg[0] += 50*CppAD::pow(vars[delta_start + t], 2);
      fg[0] += 1*CppAD::pow(vars[a_start + t], 2);
    }

    // Minimize the value gap between sequential actuations. Heigher weight
    // placed on steering to prevent large changes in steering wheel angle from step to step
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 + v0 * CppAD::tan(delta0) / Lf * dt);
